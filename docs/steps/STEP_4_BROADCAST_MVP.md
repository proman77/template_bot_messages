...
---
## 5. Оптимизация и Исправления — ✅ ВЫПОЛНЕНО
*   **Dependency Injection:** Обновлен файл `app/services/scheduler/dependencies.py`. Теперь задачи используют объекты `bot` и `db_pool` напрямую из `Taskiq State`, что гарантирует использование правильного цикла событий (Event Loop) и экономит ресурсы.
*   **Database Tables:** Исправлены ошибки доступа к данным в `UsersTable` и `BroadcastTable` (переход на использование `.data` и `.to_models()`).
*   **Тестирование:** Система успешно протестирована полным циклом: создание кампании -> постановка в очередь -> отправка сообщения -> логирование результата.

---
## 6. Особенности работы на Windows (Compatibility) — ✅ ВЫПОЛНЕНО
...
1.  [x] **Админ-интерфейс:** Реализован поток создания рассылки через `aiogram-dialog` с поддержкой медиа и кнопок.
2.  [x] **Асинхронность:** Рассылка выполняется в фоновом режиме через Taskiq, не блокируя основной поток бота.
3.  [x] **Сегментация:** Система корректно фильтрует получателей по выбранному языку (ru/en/all).
4.  [x] **Обработка ошибок:** Реализовано автоматическое обновление статуса пользователя (`is_active = False`) при получении ошибки о блокировке бота (`Forbidden`).
5.  [x] **Завершение:** Статус кампании автоматически переходит в `completed` после завершения обработки всех задач в очереди.